...
//     if (Object.keys(data) == "variants") {
//       let xyz = Object.values(data);
//       let Booksdata = await Product.find({
//         "variants.name": `${xyz}`,
//       }).select(t1);
//       console.log(Booksdata);
//       return res
//         .status(200) // Update the response status code to 200
//         .send({ status: true, message: "success", data: Booksdata });
//     }
//     let Booksdata = await Product.find(data).select(t1);

//     if (Booksdata.length == 0) {
//       return res
//         .status(404) // Update the response status code to 404
//         .send({ status: false, message: "No matching products found" });
//     }

//     res.status(200).send({ status: true, message: "success", data: Booksdata });
//   } catch (error) {
//     return res
//       .status(500) // Update the response status code to 500
...
exports.searchProducts = async (req, res) => {
  try {
    ...
    const products = await Product.find({ 'variants.name': variants });

    if (products.length === 0) {
      return res.status(404).json({ status: false, message: 'No matching products found' });
    }

    res.json({ status: true, message: 'success', data: products });
  } catch (error) {
    console.log(error);
